...
const member = {
  ...
  * 로그인 처리
  *     회원 아이디, 비번
  *       1. 회원아이디를 통해서 가입된 회원 정보를 조회
  *       2. 회원 정보의 비밀번호 해시와 입력된 비밀번호를 비교 -> 일치하면
  *       3. 일치하면
  *             세션에 비밀번호가 일치하는 회원의 아이디를 저장
  *             req.session의 하위 속성으로 등록(req객체 -> 세션을 사용하기 위해)
  *       4. 전역 적용 미들웨어 세션에 저장된 아이디를 통해
  *          회원 정보를 조회 -> 전역에 유지
  */
  ...
  */
  get : async function(memId){
    try {
      ...
      const filePath = path.join(__dirname,"../data/memaber",memId + ".json");
