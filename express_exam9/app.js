...
const upload = multer({
  storage : multer.diskStorage({
    destination : function(req, file, done) { //파일이 저장될 폴더명
      //upload
      done(null, path.join(__dirname, "upload"));
    },
    filename : function(req, file, done) { //파일이 저장되는 파일명 관련 설정
      /*
        file 객체 - 업로드된 파일 정보
                  - file.originalname - 원 파일명

                  - 파일 중복 방지
                  - timestamp -> 숫자 -> 1970.1.1 자정 -> 밀리초단위로 센 숫자
                  파일명_timestamp.확장자
                  new Date().getTime()
                  Date.now()
